...
## 메인 실습: `src/App.jsx`에서 세 방식 비교

같은 **`Header`·`MenuCard`** 컴포넌트에 `variant`만 바꿔 **일반 CSS / CSS Modules / Tailwind** 를 **한 화면에 나란히** 보여 주는 것이 1강 기본 동작이다. `npm run dev` 로 확인한다.
...
| Tailwind | `Header.jsx` / `MenuCard.jsx` 의 `variant === 'tailwind'` 분기 + `vite.config.js` 의 `@tailwindcss/vite` |
