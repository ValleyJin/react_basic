# React·풀스택 입문 시리즈 (~16강)

같은 주제(가상의 카페 BeanLog의 홈페이지 구현)로 **기능을 조금씩 붙여** 갑니다. **1~5강**을 마치면 **여러 페이지로 구성된 작은 웹사이트**가 되고, **6강 이후**에는 IoC·서버·DB·관리자 UI·(선택) DB 심화·캡스톤·UI 블록 실전(16강)까지 확장할 수 있습니다.

**Git 무시 규칙:** `node_modules/`, `dist/`, `target/`, `.env` 등은 **이 `lessons` 폴더의 부모(저장소 루트)에 있는 `.gitignore`** 한 파일에서 통합해 둔다. 강별로 `.gitignore`를 두지 않는다.
...
## 2. 백앤드: Spring Boot · PostgreSQL · 관리자 기능
...
| `lesson-16-shadcn` | Next.js + shadcn Blocks(`dashboard-01`) 실전: 조립형 UI, 차트(Recharts), 테이블(TanStack), DnD(dnd-kit) 구조 읽기 |

## 3. DB 구현: PostgreSQL 심화 트랙 (선택 · 12~14강)
...
**16강**은 10강 또는 15강 이후에, **관리자 화면 UI를 더 현대적인 블록 조립 방식으로 읽고 싶을 때** 보면 좋다(서버 없이 프론트만 실행 가능).  
...
- **16강(선택):** **Next.js + shadcn Blocks**를 실제로 가져와, 대시보드 화면의 **컴포넌트 조립·상태 흐름 읽기**를 연습한다.

**의도적으로 다른 두 강 — 10강과 15강**둘은 **서로를 이어 붙여 한 번에 돌리는 강**이 아니라, **각각 완결**된다. **같은 도메인을 “풀스택 한 레포”로만 정리해 보고 싶다면 15강**을 보면 된다.
...
브라우저 주소는 `http://localhost:3000`이며, 이 프로젝트는 루트(`/`) 접속 시 `/dashboard`로 이동하도록 구성되어 있다.
